...
def prepare_model_settings(label_count, sample_rate, clip_duration_ms,
                           ...
                           dct_coefficient_count, num_layers, num_units, use_attn, attn_size):
...
def create_lstm_model(fingerprint_input, model_settings, is_training):
    ...
    attn_size = model_settings['attn_size']
    attn_type = 'relu'#model_settings['relu']

    fingerprint_2d = tf.reshape(fingerprint_input, [-1, input_time_size, input_frequency_size])
    #fingerprint_2d = fingerprint_2d[:, :, 1:]
    def lstm_cell(lstm_size): return tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.BasicLSTMCell(lstm_size), output_keep_prob=dropout_prob)
    def bilstm_cell(lstm_size): return tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.BasicLSTMCell(lstm_size/2), output_keep_prob=dropout_prob)
